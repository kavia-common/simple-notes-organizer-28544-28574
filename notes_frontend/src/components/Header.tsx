...
export default function Header(): React.ReactElement {
  ...
  const handleAction = (action: "search" | "info") => () => {
    // In a future iteration, wire to real features
  ...
  return (
    <header className="app-header" role="banner" aria-label="Application header">
      ...
      <div className="header-actions" role="group" aria-label="Header actions">
        <button className="icon-button" aria-label="Search" onClick={handleAction("search")}>
        ...
        <button className="icon-button" aria-label="Info" onClick={handleAction("info")}>
          <svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true">
            <path
